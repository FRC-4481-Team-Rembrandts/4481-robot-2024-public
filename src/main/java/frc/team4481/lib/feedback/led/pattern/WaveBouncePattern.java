 ...
 */
@Deprecated
public class WaveBouncePattern implements LEDPattern {
    private double previousTime = Timer.getFPGATimestamp();
    private double timeContainer = 0;
    ...
    public void updateBuffer(AddressableLEDBuffer buffer, LEDStrip strip) {
        ...
        if(timeContainer >= strip.getPatternDuration() || timeContainer <= 0) {
            directionReversed = !directionReversed;
        }

//        System.out.println(timeContainer);

        if(directionReversed) {
            timeContainer -= deltaTime;
        } else {
            timeContainer += deltaTime;
        }
        ...
        double timeOffset = timeContainer * timeWavelength;
